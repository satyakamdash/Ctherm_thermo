...
f = open("./examples/res_ctherm_ref_exp_max10/mdf_etoh.json", "r")
s = f.read()
data = json.loads(s)
f.close()
...
nadphRatio2Test = sorted([float(k) for k in data["P2_ATP_PFK_ATPratio"].keys()])
downstreamMaxConc = sorted([float(k) for k in data["P2_ATP_PFK_ATPratio"][str(nadphRatio2Test[0])].keys()])
...
for k1, v1 in data.iteritems():
	...
	KARI = "KARI_NADH" if k1 == "P2_ATP_PFK_ATPratio" else "KARI_NADPH"
